...
class UniformSampleFrames:
    ...
    def __init__(self,
                 ...
                 p_interval=1,
                 seed=255,
                 keyframe_temporal_segment = {'keyframes':0, 'temporal_segment':False, 'resort':False},
                 **deprecated_kwargs):
        ...
        self.keyframe_temporal_segment = keyframe_temporal_segment
    ...
    def _get_train_clips(self, num_frames, clip_len, results):
        ...
        for clip_idx in range(self.num_clips):
            ...
            if 0 < self.keyframe_temporal_segment['keyframes']:
                if num_frames < self.keyframe_temporal_segment['keyframes']:
                    start = np.random.randint(0, num_frames)
                    keyinds = np.arange(start, start + self.keyframe_temporal_segment['keyframes'])
                else:
                    samplekeys = results.get('samplekey')
                    samplekeys = samplekeys.astype(np.float32)
                    keyinds = self._get_keyinds(samplekeys)
                    if self.keyframe_temporal_segment['temporal_segment'] == True:
                        keyinds = self._get_temporal_segmented_keyinds(keyinds, num_frames, self.keyframe_temporal_segment['keyframes'])
                    else:
                        keyinds = keyinds[:self.keyframe_temporal_segment['keyframes']]
                        keyinds = np.sort(keyinds)
                inds = np.concatenate([inds,keyinds])
            if self.keyframe_temporal_segment['resort'] == True:
                    inds = np.sort(inds)
    ...
    def _get_test_clips(self, num_frames, clip_len, results):
        ...
        for i in range(self.num_clips):
            ...
            if 0 < self.keyframe_temporal_segment['keyframes']:
                if num_frames < self.keyframe_temporal_segment['keyframes']:
                    start_ind = i if num_frames < self.num_clips else i * num_frames // self.num_clips
                    keyinds = np.arange(start_ind, start_ind + self.keyframe_temporal_segment['keyframes'])
                else:
                    samplekeys = results.get('samplekey')
                    samplekeys = samplekeys.astype(np.float32)
                    keyinds = self._get_keyinds(samplekeys)
    ...
    def _get_keyinds(self, samplekey):
        samplekey = samplekey[0]
        sorted_indices = np.argsort(samplekey)
        return sorted_indices


    def _get_temporal_segmented_keyinds(self,keyinds, num_frames,  keyframes):
        interval = num_frames//keyframes
        segmented_keyinds = []
        for i in range(keyframes):
            temporal_segmented_keyinds = [x for x in keyinds if i*interval <= x < (i+1)*interval]
            segmented_keyinds.append(temporal_segmented_keyinds[0])
        segmented_keyinds = np.array(segmented_keyinds)
        segmented_keyinds = segmented_keyinds.reshape(-1)
        
        return segmented_keyinds

...
class JointToBone:
    def __init__(self, dataset='nturgb+d', target='keypoint'):
        ...
        self.target = target
        if self.dataset not in ['nturgb+d', 'openpose', 'coco', 'handmp', 'openpose_25']:
            raise ValueError(
                f'The dataset type {self.dataset} is not supported')
        if self.dataset == 'nturgb+d': # self.center = 20
        ...
        elif self.dataset == 'openpose': # self.center = 1
            self.pairs = ((0, 1), (1, 1), (2, 1), (3, 2), (4, 3), (5, 1), (6, 5), (7, 6), (8, 2), (9, 8), (10, 9),
                          (11, 5), (12, 11), (13, 12), (14, 0), (15, 0), (16, 14), (17, 15))
        elif self.dataset == 'coco': # self.center = 0
            ...
            self.pairs = ((0, 0), (1, 0), (2, 1), (3, 2), (4, 3), (5, 0), (6, 5), (7, 6), (8, 7), (9, 0), (10, 9),
                          ...
                          (20, 19))
        elif self.dataset == 'openpose_25': # self.center = 8
            self.pairs = ((0, 1), (1, 8), (2, 1), (3, 2), (4, 3), (5, 1), (6, 5), (7, 6), (8, 8), (9, 8), (10, 9), 
                          (11, 10), (12, 8), (13, 12), (14, 13), (15, 0), (16, 0), (17, 15), (18, 16), (19, 14), 
                          (20, 19), (21, 14), (22, 11), (23, 22), (24, 11))

    def __call__(self, results):
        if self.target in results:
            return results
        else:
            if 'bone' in results:
                results[self.target] = results.pop('bone')
            else:
                ...
                for v1, v2 in self.pairs:
                    ...
                    if C == 3 and self.dataset in ['openpose', 'coco', 'handmp', 'openpose_25']:
...
class ToMotion:
    ...
    def __call__(self, results):
        ...
        if C == 2 and self.source in ['cosine','c']:
            ...
            motion[:, :T - 1, :, 1] = score
        if C == 3 and self.dataset in ['openpose', 'coco', 'openpose_25']:
...
class JointToCosine:
    def __init__(self, dataset='nturgb+d', target='cosine'):
        self.dataset = dataset
        self.target = target
        if self.dataset not in ['nturgb+d', 'openpose', 'coco','openpose_25']:
            raise ValueError(
                f'The dataset type {self.dataset} is not supported')
        if self.dataset == 'nturgb+d': # self.center = 20
            self.pairs = [(0, 1), (1, 20), (2, 20), (3, 2), (4, 20), (5, 4), (6, 5), (7, 6), (8, 20), (9, 8),
    ...
    def __call__(self, results):
        if self.target in results:
            ...
            if 'cosine' in results:
                results[self.target] = results.pop('cosine')
            else:
                if 'bone' in results:
                    bone = results['bone']
...
class GenSkeFeat:
    def __init__(self, dataset='nturgb+d', feats=['j'], axis=-1):
        ...
        self.axis = axis
        ops = []
        if 'c' in feats or 'cm' in feats:
            ops.append(JointToCosine(dataset=dataset, target='c'))
        ...
        if 'cm' in feats:
            ops.append(ToMotion(dataset=dataset, source='c', target='cm'))

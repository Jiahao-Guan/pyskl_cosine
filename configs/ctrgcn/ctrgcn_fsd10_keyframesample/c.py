model = dict(
    ...
    backbone=dict(
		type='CTRGCN',
        graph_cfg=dict(layout='openpose_25', mode='spatial')),
    cls_head=dict(type='GCNHead', num_classes=10, in_channels=256))

dataset_type = 'PoseDataset'
ann_file = 'data/fsd10/fsd10_kbc.pkl'
train_pipeline = [
    dict(type='GenSkeFeat', dataset='openpose_25', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=100, keyframe_temporal_segment=dict(
...
val_pipeline = [
    dict(type='GenSkeFeat', dataset='openpose_25', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=100, num_clips=1, keyframe_temporal_segment=dict(
            keyframes=25, temporal_segment=True, resort=True)),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
...
test_pipeline = [
    dict(type='GenSkeFeat', dataset='openpose_25', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=100, num_clips=10, keyframe_temporal_segment=dict(
            keyframes=25, temporal_segment=True, resort=True)),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
...
# runtime settings
log_level = 'INFO'
work_dir = './work_dirs/ctrgcn/ctrgcn_fsd10_keyframesample/c4_4'

...
ann_file = 'data/gym/gym_kbc.pkl'
train_pipeline = [
    dict(type='GenSkeFeat', dataset='coco', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=50, keyframe_temporal_segment=dict(
...
val_pipeline = [
    dict(type='GenSkeFeat', dataset='coco', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=50, num_clips=1, keyframe_temporal_segment=dict(
            keyframes=25, temporal_segment=True, resort=True)),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
...
test_pipeline = [
    dict(type='GenSkeFeat', dataset='coco', feats=['c']),
    #dict(type='PreNormalize2D',mode='auto'),
    dict(type='UniformSample', clip_len=50, num_clips=10, keyframe_temporal_segment=dict(
            keyframes=25, temporal_segment=True, resort=True)),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
...
# runtime settings
log_level = 'INFO'
work_dir = './work_dirs/ctrgcn/ctrgcn_gym_keyframesample/c_clip50'
